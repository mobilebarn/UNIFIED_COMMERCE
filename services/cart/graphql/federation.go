	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Cart", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Cart", ErrTypeNotFound))
			break
		}
		return "findCartByID", nil
	}
	return "", fmt.Errorf("%w for Cart due to %v", ErrTypeNotFound,
