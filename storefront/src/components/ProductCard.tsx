...
export function ProductCard({ product }: ProductCardProps) {
  ...
  // Placeholder for rating since it's not in the schema
  const rating = 4.5; 
  const reviews = Math.floor(Math.random() * 200) + 50; // Random reviews

  // Handle both mock data (name) and GraphQL data (title)
    ...
    return (
      <div className="flex items-center">
        {[...Array(5)].map((_, i) => (
          <svg 
            key={i}
            className={`w-4 h-4 ${i < Math.floor(rate) ? 'text-yellow-400' : 'text-gray-300'}`}
            fill="currentColor" 
            viewBox="0 0 20 20" 
        ...
        <span className="ml-1 text-sm text-gray-500">({rate})</span>
      </div>
    );
  };
