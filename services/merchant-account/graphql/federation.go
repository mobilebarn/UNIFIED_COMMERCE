...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Merchant":
		resolverName, err := entityResolverNameForMerchant(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Merchant": %w`, err)
		}
		switch resolverName {
		...
		case "findMerchantByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findMerchantByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindMerchantByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Merchant": %w`, err)
			}
...
func entityResolverNameForMerchant(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Merchant", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Merchant", ErrTypeNotFound))
			break
		}
		return "findMerchantByID", nil
	}
	return "", fmt.Errorf("%w for Merchant due to %v", ErrTypeNotFound,
