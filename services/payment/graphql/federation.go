...
func (ec *executionContext) resolveEntity(
	...
	case "PaymentMethod":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "PaymentMethod": %w`, err)
		}
		switch resolverName {
		...
		case "findPaymentMethodByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPaymentMethodByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindPaymentMethodByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "PaymentMethod": %w`, err)
			}
	...
	case "Refund":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Refund": %w`, err)
		}
		switch resolverName {
		...
		case "findRefundByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findRefundByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindRefundByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Refund": %w`, err)
			}
...
func entityResolverNameForPaymentMethod(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for PaymentMethod", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for PaymentMethod", ErrTypeNotFound))
			break
		}
		return "findPaymentMethodByID", nil
	}
	return "", fmt.Errorf("%w for PaymentMethod due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForRefund(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Refund", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Refund", ErrTypeNotFound))
			break
		}
		return "findRefundByID", nil
	}
	return "", fmt.Errorf("%w for Refund due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
